...
| Day 15 | 2026-06-19 | Complete | [Day 15.md](Day%2015.md) | [images/Day 15](images/Day%2015/) | 8051 serial port operation, full duplex communication, receive buffering, `SBUF` transmit/receive paths, `SCON` bit layout, `REN`, `TI/RI`, `SM0/SM1` serial modes, Mode 0 shift-register operation, Mode 1 UART framing, Mode 2 fixed-baud 9-bit UART, Mode 3 variable-baud 9-bit UART, `SM2` multiprocessor filtering, `TXD/RXD`, baud rate, `PCON/SMOD` baud-rate doubling, Timer 1/`TH1` baud-rate generation, and practical `TMOD=20H`/`SCON=50H`/`TR1` Mode 1 setup. |
